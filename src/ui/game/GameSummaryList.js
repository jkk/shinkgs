...
class GameSummaryListItem extends Component<GameSummaryListItemProps> {
  render() {
    ...
    let className = 'GameSummaryList-item' + (
      won ? ' GameSummaryList-item-won' :
        (player && winningColor ? ' GameSummaryList-item-lost' : '')
    );
...
type Props = {
  ...
  onSelect: (game: GameSummary, channelId?: string, loadPrivate?: boolean) => any
};
...
export default class GameSummaryList extends Component<Props, State> {
  ...
  render() {
    ...
    let {fullRender, showLoadGameModal, selectedGame, loadPrivate} = this.state;
  ...
  _getRoomList = (game?: GameSummary): Array<Element<'div'>> => {
    ...
    const rooms = roomIds.map(id => (
      <div key={id} className='GameSummaryList-load-game-item'>
        <A button onClick={() => { game && onSelect(game, id, loadPrivate); }}>
          {roomsById[id].name}
        </A>
      </div>
    ));
